# Graduated from research/utils.py — keep in sync manually
"""
...
def encode_speech(model, audio_features, dtype, pre_encoded=False):
    """Process audio through Whisper encoder and adapter.

    If pre_encoded=True, audio_features are already Whisper encoder outputs
    and only the adapter is applied.
    """
    if pre_encoded:
        return model.adapter(audio_features.to(dtype=dtype))
    with torch.no_grad():
...
def prepare_batch(
    ...
    speech_embeds = encode_speech(model, audio_features, dtype, pre_encoded=pre_encoded)
